...
struct TodaysJourneyItemView: View {
  ...
  var body: some View {
    ...
    .padding(.top, isFirst ? 8 : 0) // Only first item needs top padding; others connect via line above
  ...
  private var timeColumn: some View {
    ...
    .padding(.top, isFirst ? 0 : 6) // Align with dot position (line above is 6pt)
  }
    ...
    VStack(spacing: 0) {
      ...
      if !isFirst {
        lineSegment(position: .above)
          .frame(height: 6) // Matches the row's top padding (reduced for tighter spacing)
      }
