...
struct JiggleAnimationModifier: ViewModifier {
  ...
  func body(content: Content) -> some View {
    content
      ...
      .rotationEffect(.degrees(isEditMode
          ? 0.2
          : 0)) // Reduced from 0.3 to 0.2 for subtler rotation
        .animation(
          isEditMode
...
struct DropViewDelegate: DropDelegate {
  ...
  func performDrop(info _: DropInfo) -> Bool {
    guard let draggedHabit else { return false }
    ...
    {
      print(
        "❌ Drop failed: fromIndex=\(fromIndex?.description ?? "nil"), toIndex=\(toIndex?.description ?? "nil")")
      return false
    }
  ...
  func dropEntered(info: DropInfo) {
       ...
       let currentIndex = items.firstIndex(of: item)
    {
      ...
      let newInsertionIndex: Int = if info.location.y > threshold {
...
struct HabitsTabView: View {
  ...
  var body: some View {
    WhiteSheetContainer(
          ...
          VStack(alignment: .leading, spacing: 8) {
            if habits.isEmpty {
              ...
              LazyVStack(spacing: 12) {
                ...
                if isEditMode, insertionIndex != nil, draggedHabit != nil,
                   insertionIndex == filteredHabits.count
                {
                  HStack {
                    Rectangle()
                      .fill(Color.accentColor)
                  ...
                  Rectangle()
                    ...
                    .overlay(
                      ...
                      RoundedRectangle(cornerRadius: 8)
                        .stroke(
                          dragOverItem == nil && draggedHabit != nil && insertionIndex ==
                        ...
                        .animation(
                          ...
                          value: dragOverItem == nil && draggedHabit != nil && insertionIndex ==
                            filteredHabits.count))
                }
              }
      ...
      .onChange(of: habits) { oldHabits, newHabits in
        ...
        print(
          "🔄 HabitsTabView: habits parameter changed from \(oldHabits.count) to \(newHabits.count)")
      ...
      .onChange(of: habitsOrder) { oldOrder, newOrder in
        ...
        if oldOrder != newOrder, newOrder.count >= oldOrder.count, onUpdateHabit != nil {
          ...
          print(
            "🔄 HabitsTabView: Habit count decreased (likely deletion), skipping reorder notification")
        }
      }
      .onDisappear {
    ...
    Group {
      if isEditMode {
        ...
        HStack(spacing: 16) {
          ...
          .animation(
            .easeOut(duration: 0.25).delay(0.1),
            value: isEditMode) // Improved timing and reduced delay
          .background(
            Circle()
              .fill(Color.clear)
          ...
          AddedHabitItem(
            ...
            .overlay(
              ...
              RoundedRectangle(cornerRadius: 8)
                .stroke(
                  dragOverItem == habit && draggedHabit != habit
                    ? Color.accentColor.opacity(0.6)
                    : Color.clear,
                  lineWidth: 2)
                .animation(
                  .easeOut(duration: 0.15),
                  value: dragOverItem == habit && draggedHabit != habit))
  ...
  private func updateDragState(dragOver: Habit?, insertion: Int?) {
    ...
    print(
      "🔄 Drag state update: dragOver=\(dragOver?.name ?? "nil"), insertion=\(insertion?.description ?? "nil")")
    ...
    dragUpdateTimer = Timer.scheduledTimer(withTimeInterval: 0.03, repeats: false) { _ in
      ...
      print(
        "✅ Drag state updated: dragOverItem=\(dragOver?.name ?? "nil"), insertionIndex=\(insertion?.description ?? "nil")")
    }
  }
