...
final class ObservabilityLogger {
    ...
    duration: TimeInterval)
  {
    ...
    logger
      .info(
        "Migration ended - userId: \(anonymizedUserId), version: \(version), status: \(status), records: \(recordsCount), duration: \(String(format: "%.2f", duration))s")
  }
  ...
  func logMigrationError(userId: String, version: Int, error: Error) {
    ...
    logger
      .error(
        "Migration error - userId: \(anonymizedUserId), version: \(version), error: \(error.localizedDescription)")
  }
  ...
  func logXPAward(userId: String, dateKey: String, xpGranted: Int, reason: String) {
    ...
    logger
      .info(
        "XP awarded - userId: \(anonymizedUserId), dateKey: \(dateKey), xp: \(xpGranted), reason: \(reason)")
  }
  ...
  func logXPRevocation(userId: String, dateKey: String, xpRevoked: Int, reason: String) {
    ...
    logger
      .info(
        "XP revoked - userId: \(anonymizedUserId), dateKey: \(dateKey), xp: \(xpRevoked), reason: \(reason)")
  }
  ...
  func logLevelUp(userId: String, oldLevel: Int, newLevel: Int, totalXP: Int) {
    ...
    logger
      .info(
        "Level up - userId: \(anonymizedUserId), level: \(oldLevel) -> \(newLevel), totalXP: \(totalXP)")
  }
  ...
  func logAuthSwitch(fromUserId: String?, toUserId: String, reason: String) {
    ...
    logger
      .info(
        "Auth switch - from: \(anonymizedFromUserId), to: \(anonymizedToUserId), reason: \(reason)")
  }
    ...
    details: String? = nil)
  {
    ...
    logger
      .info(
        "Data consistency check - userId: \(anonymizedUserId), type: \(checkType), status: \(status)\(detailsStr)")
  }
  ...
  func logDenormalizedFieldAccess(userId: String, fieldName: String, accessType: String) {
    ...
    logger
      .warning(
        "Denormalized field access - userId: \(anonymizedUserId), field: \(fieldName), type: \(accessType)")
  }
  ...
  func logSlowOperation(operation: String, duration: TimeInterval, threshold: TimeInterval) {
    logger
      .warning(
        "Slow operation - operation: \(operation), duration: \(String(format: "%.2f", duration))s, threshold: \(String(format: "%.2f", threshold))s")
  }

  /// Log memory usage
  func logMemoryUsage(operation: String, memoryMB: Double) {
    logger
      .info("Memory usage - operation: \(operation), memory: \(String(format: "%.1f", memoryMB))MB")
  }
  ...
  func logXPMutationViolation(caller: String, function: String, userId: String) {
    ...
    logger
      .error(
        "XP mutation violation - caller: \(caller), function: \(function), userId: \(anonymizedUserId)")
  }
  ...
  func logDataCorruption(userId: String, corruptionType: String, details: String) {
    ...
    logger
      .error(
        "Data corruption detected - userId: \(anonymizedUserId), type: \(corruptionType), details: \(details)")
  }
...
extension ObservabilityLogger {
  ...
  func logFeatureFlagChange(flagName: String, oldValue: Bool, newValue: Bool, userId: String) {
    ...
    logger
      .info(
        "Feature flag changed - flag: \(flagName), \(oldValue) -> \(newValue), userId: \(anonymizedUserId)")
  }
  ...
  func logTestExecution(testName: String, passed: Bool, duration: TimeInterval) {
    ...
    logger
      .info(
        "Test executed - test: \(testName), status: \(status), duration: \(String(format: "%.2f", duration))s")
  }
