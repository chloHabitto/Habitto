...
// MARK: - Semantic Version Validator
...
struct SemverValidator {
    ...
    static func isMonotonic(_ currentVersion: String, previousVersion: String) -> Bool {
        return compare(currentVersion, previousVersion) != .orderedAscending
    }
}
...
extension CrashSafeHabitStore {
    func validateStorageInvariantsWithSemver(_ container: HabitDataContainer, previousVersion: String? = nil) throws {
        // 1. Basic invariants
        ...
        if let previous = previousVersion {
            guard SemverValidator.isMonotonic(container.version, previousVersion: previous) else {
                throw HabitStoreError.dataIntegrityError("Non-monotonic version: \(previous) -> \(container.version)")
            }
        }
