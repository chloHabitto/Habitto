...
final class BackupTestingSuite: ObservableObject {
    ...
    func runAllTests() async {
        ...
        logger.info("🧪 Backup testing completed: \(self.testResults.filter { $0.success }.count)/\(self.testResults.count) tests passed")
    }
    ...
    private func testBackupRestoration() async throws {
        ...
        guard restoreResult.success else {
            throw BackupTestError.restorationFailed("Backup restoration failed: \(restoreResult.details.joined(separator: ", "))")
        }
    ...
    private func testErrorHandling() async throws {
        ...
        do {
            let _ = try await backupManager.restoreFromData(invalidData)
        ...
        do {
            let _ = try await backupManager.restoreFromData(invalidBackupData)
    ...
    private func testStorageCoordinator() async throws {
        ...
        guard storageCoordinator.enableiCloudBackup || storageCoordinator.enableGoogleDriveBackup || storageCoordinator.enableLocalBackup else {
    ...
    private func testBackgroundTasks() async throws {
        ...
        let _ = BackupScheduler.loadScheduleConfig()
