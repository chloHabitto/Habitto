...
final class BackupStorageCoordinator: ObservableObject {
    ...
    private func saveConfiguration() {
        UserDefaults.standard.set(preferredStorageProvider.rawValue, forKey: "backup_preferred_provider")
    ...
    private func performBackupWithProvider(_ provider: StorageProvider) async throws -> BackupProviderResult {
        let backupData = try await backupManager.createBackupData()
        let filename = generateBackupFilename()
        
        switch provider {
        case .iCloud:
            let uploadResult = try await cloudStorageManager.uploadToiCloud(backupData, filename: filename)
        ...
        case .googleDrive:
            let uploadResult = try await googleDriveManager.uploadToGoogleDrive(backupData, filename: filename)
            return BackupProviderResult(
                provider: .googleDrive,
                success: true,
                error: nil,
    ...
    func restoreBackup(_ backupFile: BackupStorageFileInfo) async throws -> RestoreResult {
        ...
        case .googleDrive:
            backupData = try await googleDriveManager.downloadFromGoogleDrive(fileId: backupFile.storagePath)
    ...
    func getConfiguration() -> BackupStorageConfiguration {
        return BackupStorageConfiguration(
...
enum BackupStorageError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidProvider:
            return "Invalid storage provider specified"
        case .noProvidersAvailable:
            return "No backup storage providers are available"
        case .restoreFailed:
