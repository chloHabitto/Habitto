...
// MARK: - Backup Schedule Configuration
struct BackupScheduleConfig: Codable {
    ...
    init(
        ...
        preferredTime: Date = Calendar.current.date(bySettingHour: 2, minute: 0, second: 0, of: Date()) ?? Date(),
...
private extension String {
    static let backupTaskIdentifier = "com.habitto.app.backup"
    static let backupRefreshIdentifier = "com.habitto.app.backup.refresh"
}
...
class BackupScheduler: ObservableObject {
    ...
    func updateSchedule(
        ...
        if let networkCondition = networkCondition {
        ...
        logger.info("Backup schedule updated: enabled=\(config.isEnabled), frequency=\(config.frequency.rawValue)")
    }
    ...
    private func calculateNextBackupDate() -> Date {
        ...
        let preferredComponents = calendar.dateComponents([.hour, .minute], from: scheduleConfig.preferredTime)
    ...
    private func updateNetworkStatus(_ path: NWPath) {
        if path.usesInterfaceType(.wifi) {
            networkStatus = .wifiOnly
        } else if path.status == .satisfied {
            networkStatus = .any
        } else {
            networkStatus = .any // Default to any when no connection
        }
    ...
    private func registerBackgroundTasks() {
        BGTaskScheduler.shared.register(forTaskWithIdentifier: .backupTaskIdentifier, using: nil) { task in
            self.handleBackupTask(task: task as! BGAppRefreshTask)
        }
        
        BGTaskScheduler.shared.register(forTaskWithIdentifier: .backupRefreshIdentifier, using: nil) { task in
