...
class PermissionManager: ObservableObject {
    ...
    var canUseCamera: Bool {
        return UIImagePickerController.isSourceTypeAvailable(.camera) && cameraPermissionStatus == .authorized
    }
    ...
    func openAppSettings() {
        ...
        if UIApplication.shared.canOpenURL(settingsUrl) {
            UIApplication.shared.open(settingsUrl) { success in
                print(success ? "✅ PermissionManager: Opened app settings" : "❌ PermissionManager: Failed to open app settings")
            }
        }
    }
}
