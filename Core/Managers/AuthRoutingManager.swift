...
final class AuthRoutingManager: ObservableObject {
  ...
  private func switchToGuestUser() async {
    ...
    // Log auth switch
    ObservabilityLogger.shared.logAuthSwitch(
    ...
    do {
      ...
      logger
        .error("AuthRoutingManager: Failed to switch to guest user: \(error.localizedDescription)")
    }
  }
  ...
  private func switchToUser(userId: String) async {
    ...
    do {
      ...
      logger
        .error(
          "AuthRoutingManager: Failed to switch to user \(userId): \(error.localizedDescription)")
    }
  }
...
extension AuthRoutingManager {
  ...
  var isAuthenticated: Bool {
    currentUserId != nil && currentUserId != "guest_user"
  }
