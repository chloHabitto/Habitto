...
// MARK: - MigrationTelemetryManager
...
class MigrationTelemetryManager: ObservableObject {
  ...
  func checkKillSwitch() async {
    do {
      ...
      print(
        "🔧 MigrationTelemetryManager: Kill switch checked - migrations \(isMigrationEnabled ? "enabled" : "disabled")")

    } catch {
      print(
        "⚠️ MigrationTelemetryManager: Failed to check kill switch, keeping migrations enabled: \(error)")
  ...
  func getTelemetrySummary() -> String {
    ...
    let recentEvents = events
      .filter { $0.timestamp > Date().addingTimeInterval(-86400) } // Last 24 hours
    ...
    let successRate = endEvents.isEmpty
      ? 0.0
      : Double(endEvents.filter { $0.success }.count) / Double(endEvents.count)
  ...
  private let killSwitchURL =
    URL(
      string: "https://raw.githubusercontent.com/chloe-lee/Habitto/main/migration-kill-switch.json")!
...
extension DataMigrationError {
  var errorDescription: String? {
    ...
    case .migrationDisabledByKillSwitch:
      "Migration disabled by remote kill switch"
    case .highFailureRateDetected:
      "Migration disabled due to high failure rate in last 24 hours"
    case .requiredStepFailed(let step, let error):
      "Required migration step '\(step)' failed: \(error.localizedDescription)"
    case .invalidRollbackVersion:
      "Cannot rollback to a version that is not older than current version"
    case .rollbackFailed(let step, let error):
      "Failed to rollback step '\(step)': \(error.localizedDescription)"
    case .postMigrationValidationFailed(let message):
      "Post-migration validation failed: \(message)"
    case .unknown:
