...
// MARK: - Enhanced Migration Telemetry Manager
...
class EnhancedMigrationTelemetryManager: ObservableObject {
    ...
    @Published var isMigrationEnabled: Bool = true
    @Published var remoteConfigLoaded: Bool = false
    ...
    struct RemoteConfig: Codable {
        ...
        init(from decoder: Decoder) throws {
            ...
            self.isMigrationEnabled = try container.decodeIfPresent(Bool.self, forKey: .isMigrationEnabled) ?? true
            self.minAppVersion = try container.decodeIfPresent(String.self, forKey: .minAppVersion)
            self.maxFailureRate = try container.decodeIfPresent(Double.self, forKey: .maxFailureRate) ?? 0.1
    ...
    struct TelemetryEvent: Codable, Identifiable {
        ...
        init(eventType: EventType, timestamp: Date, version: String, duration: TimeInterval? = nil, errorCode: String? = nil, datasetSize: Int? = nil, success: Bool, deviceInfo: DeviceInfo) {
            self.id = UUID()
    ...
    func checkMigrationEnabled() async -> Bool {
        ...
        if isFailureRateTooHigh() {
            await recordEvent(.killSwitchTriggered, duration: nil, errorCode: "high_failure_rate", datasetSize: nil, success: false)
            return false
        }
    ...
    func recordEvent(_ eventType: TelemetryEvent.EventType, duration: TimeInterval? = nil, errorCode: String? = nil, datasetSize: Int? = nil, success: Bool) async {
