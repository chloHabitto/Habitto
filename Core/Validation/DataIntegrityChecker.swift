...
class DataIntegrityChecker: ObservableObject {
  ...
  private func getSuggestedFix(for error: ValidationError) -> String {
    switch error.field {
    case "name":
      "Enter a valid habit name (2-50 characters)"
    case "description":
      "Keep description under 200 characters"
    case "icon":
      "Select a valid system icon"
    case "schedule":
      "Select a valid schedule format"
    case "goal":
      "Enter a valid goal number"
    case "streak":
      "Recalculate streak based on completion history"
    case "completionHistory":
      "Remove invalid completion dates"
    case "difficultyHistory":
      "Remove invalid difficulty values"
    default:
      "Review and correct the input"
    }
  }
...
class DataIntegrityMonitor: ObservableObject {
  ...
  func startMonitoring(habits: @escaping @MainActor () -> [Habit], interval: TimeInterval = 300) {
    // 5 minutes
    stopMonitoring()
