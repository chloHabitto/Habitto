...
struct WeekPickerModal: View {
  ...
  init(selectedWeekStartDate: Binding<Date>, isPresented: Binding<Bool>) {
    ...
    let weekStart = calendar.dateInterval(of: .weekOfYear, for: selectedWeekStartDate.wrappedValue)?
      .start ?? selectedWeekStartDate.wrappedValue
  ...
  var body: some View {
    ...
    .onAppear {
      ...
      let calendar = AppDateFormatter.shared.getUserCalendar()
      let weekStart = calendar.dateInterval(of: .weekOfYear, for: selectedWeekStartDate)?
  ...
  private func weekRangeText(from range: ClosedRange<Date>) -> String {
    AppDateFormatter.shared.formatWeekRange(startDate: range.lowerBound, endDate: range.upperBound)
  }
  ...
  private func resetToCurrentWeek() {
    ...
    let today = Date()
    let currentWeekStart = calendar.dateInterval(of: .weekOfYear, for: today)?.start ?? today
    let currentWeekEnd = calendar
      .date(byAdding: .day, value: 6, to: currentWeekStart) ?? currentWeekStart
