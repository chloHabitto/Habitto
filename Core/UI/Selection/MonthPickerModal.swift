...
struct MonthPickerModal: View {
    ...
    private var isCurrentMonthSelected: Bool {
        ...
        let selectedMonth = calendar.dateInterval(of: .month, for: tempSelectedMonth)?.start ?? tempSelectedMonth
...
struct MonthPicker: View {
    ...
    init(selectedMonth: Binding<Date>) {
        ...
        self._currentYear = State(initialValue: components.year ?? calendar.component(.year, from: Date()))
        self._currentMonth = State(initialValue: components.month ?? calendar.component(.month, from: Date()))
    }
