...
enum CalendarGridComponents {
    ...
    var body: some View {
      LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 7), spacing: 12) {
        ...
        ForEach(0 ..< firstDayOfMonth, id: \.self) { index in
          let previousMonthDay = CalendarGridComponents.getPreviousMonthDay(
            index: index,
        ...
        ForEach(1 ... daysInMonth, id: \.self) { day in
          ...
          let isSelected = CalendarGridComponents.isSelected(
            day: day,
            currentDate: currentDate,
            selectedDate: selectedDate)
          let isVacationDay = VacationManager.shared
            .isVacationDay(CalendarGridComponents.getDateForDay(
              day: day,
              currentDate: currentDate))
  ...
  static func getPreviousMonthDay(index: Int, firstDayOfMonth: Int, currentDate: Date) -> Int {
    ...
    guard let overflowDate = calendar.date(
      byAdding: .day,
      value: -daysToSubtract,
      to: firstDayOfCurrentMonth) else
    {
      return 1
    }
