...
struct TimeInsightsSection: View {
  ...
  private func enhancedIndividualHabitTimeCard(habit _: Habit) -> some View {
    VStack(spacing: 0) {
      // Main content
      HStack(spacing: 20) {
...
#Preview {
  TimeInsightsSection(habit: nil as Habit?, completionRecords: [] as [CompletionRecordEntity])
}
