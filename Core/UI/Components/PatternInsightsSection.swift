...
struct PatternInsightsSection: View {
  ...
  private func enhancedIndividualHabitPatternCard(habit _: Habit) -> some View {
    VStack(spacing: 0) {
      // Main content
      HStack(spacing: 20) {
...
#Preview {
  PatternInsightsSection(
    habit: nil as Habit?,
    completionRecords: [] as [CompletionRecordEntity],
    difficultyLogs: [] as [DifficultyLogEntity])
}
