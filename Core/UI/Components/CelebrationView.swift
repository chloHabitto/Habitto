...
enum ConfettiShapeType {
    case circle, star, heart, diamond
}
...
struct FilledCheckmarkShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        let strokedPath = path.strokedPath(StrokeStyle(lineWidth: thickness, lineCap: .round, lineJoin: .round))
