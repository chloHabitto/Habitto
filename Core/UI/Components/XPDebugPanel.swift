    ...
    VStack(alignment: .leading, spacing: 12) {
      ...
      if isRunningDiagnostic {
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          DebugRow(
            label: "Today's award:",
            value: debugInfo.todaysAwardStatus,
            isError: !debugInfo.todaysAwardGranted)
          DebugRow(
            label: "Auth status:",
            value: debugInfo.authStatus,
            isError: !debugInfo.isAuthenticated)
          DebugRow(
            label: "Migration:",
            value: debugInfo.migrationStatus,
            isError: !debugInfo.migrationCompleted)
        }
    ...
    Task {
      await MainActor.run {
        ...
        do {
          ...
          debugInfo.syncStatus = debugInfo.isInSync
            ? "✅ In sync"
            : "❌ Out of sync by \(abs(debugInfo.xpManagerTotalXP - debugInfo.swiftDataTotalXP)) XP"
          ...
          for (index, award) in awards.enumerated() {
            print(
              "    \(index + 1). dateKey: \(award.dateKey), xpGranted: \(award.xpGranted), userId: \(award.userId)")
          }
    ...
    Task {
      await MainActor.run {
        do {
          ...
          for (index, award) in awards.enumerated() {
            print(
              "🧪 TEST:   Award \(index + 1): id=\(award.id), xpGranted=\(award.xpGranted), createdAt=\(award.createdAt)")
          }

          // Test the unique constraint
          let isUnique = DailyAward.validateUniqueConstraint(
            userId: userId,
            dateKey: todayKey,
            in: modelContext)
      ...
      await MainActor.run {
        ...
        for level in 1 ... 20 {
          ...
          let status = level <= currentLevel
            ? "✅ ACHIEVED"
            : (level == currentLevel + 1 ? "🎯 NEXT" : "⏳ FUTURE")
        ...
        do {
          ...
          print(
            "  Average XP per Award: \(allAwards.isEmpty ? 0 : totalAwardedXP / allAwards.count)")
          ...
          if !allAwards.isEmpty {
            ...
            let daysSinceFirst = Calendar.current.dateComponents(
              [.day],
              from: firstAward.createdAt,
              to: Date()).day ?? 0
            let averageXPPerDay = daysSinceFirst > 0
              ? Double(totalAwardedXP) / Double(daysSinceFirst)
              : 0
      ...
      await MainActor.run {
        ...
        do {
          ...
          if allAwards.isEmpty {
            ...
            print(
              "  XP Sources Match: \(totalAwardedXP == currentXP ? "✅ YES" : "❌ NO - BUG DETECTED!")")

            print("\n🔍 XP INVESTIGATION: Daily Award Breakdown")
            for (index, award) in sortedAwards.enumerated() {
              print(
                "  Award \(index + 1): \(award.dateKey) - \(award.xpGranted) XP - \(award.createdAt)")
            }
            ...
            print(
              "  Usage Matches XP: \(expectedDays == actualDays ? "✅ YES" : "❌ NO - BUG DETECTED!")")
        ...
        if currentXP == 0 {
          ...
          print(
            "  📊 You have \(currentXP) XP - equivalent to \(daysOfUsage) days of completing all habits")
          print("  🎯 This should put you at level \(expectedLevel)")

          if currentLevel != expectedLevel {
            print(
              "  ⚠️ LEVEL MISMATCH: You're level \(currentLevel) but should be level \(expectedLevel)")
          ...
          if allAwards.isEmpty {
            ...
            let correctXP = allAwards.reduce(0) { total, award in
              ...
              if award.xpGranted == 50 {
                ...
                print(
                  "🔧 Found non-standard award: \(award.dateKey) - \(award.xpGranted) XP (likely level-up bonus)")
  ...
  private func fixYesterdayXP() {
    ...
    Task {
      await MainActor.run {
        ...
        print(
          "🔧 All habits completed yesterday: \(allCompleted) (\(completedHabits)/\(habits.count))")
...
struct XPDebugInfo {
  var currentUserId = "Loading..."
  var xpManagerTotalXP = 0
  var swiftDataTotalXP = 0
  var isInSync = false
  var syncStatus = "Loading..."
  var dailyAwardCount = 0
  var lastAwardDate = "Loading..."
  var todaysAwardGranted = false
  var todaysAwardStatus = "Loading..."
  var isAuthenticated = false
  var authStatus = "Loading..."
  var migrationCompleted = false
  var migrationStatus = "Loading..."
}
