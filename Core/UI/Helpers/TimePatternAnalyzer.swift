...
class TimePatternAnalyzer {
    ...
    static func analyzeOptimalTimeOfDay(for habit: Habit, completionRecords: [CompletionRecordEntity]) -> TimeOfDayInsight? {
    ...
    static func analyzeGlobalTimePatterns(habits: [Habit], completionRecords: [CompletionRecordEntity]) -> GlobalTimeInsight? {
        let recordsWithTime = completionRecords.filter { $0.timestamp != nil && $0.progress > 0 }
    ...
    static func analyzeTimeConsistency(for habit: Habit, completionRecords: [CompletionRecordEntity]) -> TimeConsistencyInsight? {
        let recordsWithTime = completionRecords.filter { $0.timestamp != nil && $0.progress > 0 }
        
        guard recordsWithTime.count >= 3 else { return nil } // Need at least 3 records for consistency analysis
        
        // Calculate standard deviation of completion times
        let completionHours = recordsWithTime.map { Calendar.current.component(.hour, from: $0.timestamp!) }
        ...
        if standardDeviation < 2.0 {
            consistencyLevel = .veryConsistent
    ...
    static func generateTimeBasedRecommendations(for habit: Habit, timeInsight: TimeOfDayInsight, consistencyInsight: TimeConsistencyInsight?) -> [TimeRecommendation] {
