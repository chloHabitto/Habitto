...
final class CloudKitSyncManager {
  ...
  private func checkCloudKitAvailability() async throws {
    ...
    case .temporarilyUnavailable:
      throw CloudKitError.networkError(NSError(
        domain: "CloudKit",
        code: -1,
        userInfo: [NSLocalizedDescriptionKey: "CloudKit temporarily unavailable"]))
  ...
  private func fetchRemoteChanges(since date: Date = Date
    .distantPast) async throws -> [CloudKitChange]
  {
    ...
    let query = CKQuery(
      recordType: "Habit",
      predicate: NSPredicate(format: "modificationDate > %@", date as NSDate))
  ...
  private func fetchLocalChanges(since date: Date = Date
    .distantPast) async throws -> [LocalChange]
  {
  ...
  private func resolveConflicts(
    remoteChanges: [CloudKitChange],
    localChanges: [LocalChange]) async throws -> [ConflictResolutionResult]
  {
    logger
      .debug(
        "Resolving conflicts between \(remoteChanges.count) remote and \(localChanges.count) local changes")

    var resolutions: [ConflictResolutionResult] = []

    for remoteChange in remoteChanges {
      if let localChange = localChanges
        .first(where: { $0.recordID == remoteChange.record.recordID })
      {
  ...
  private func resolveConflict(
    remoteChange: CloudKitChange,
    localChange: LocalChange) async throws -> ConflictResolutionResult
  {
