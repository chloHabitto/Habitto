...
// MARK: - CloudKit Conflict Resolver
...
class CloudKitConflictResolver: ObservableObject {
    ...
    func resolveConflict(_ conflict: ConflictRecord, using resolution: ConflictResolution) async -> ResolveResult {
        isResolving = true
    ...
    private func performResolution(_ conflict: ConflictRecord, using resolution: ConflictResolution) async throws -> CloudKitHabit {
        switch resolution {
        case .useLocal:
            return conflict.localRecord
            
        case .useRemote:
            return conflict.remoteRecord
            
        case .merge:
            return try mergeRecords(local: conflict.localRecord, remote: conflict.remoteRecord)
            
        case .recalculate:
            return try recalculateRecord(local: conflict.localRecord, remote: conflict.remoteRecord)
        }
    }
    ...
    private func recalculateRecord(local: CloudKitHabit, remote: CloudKitHabit) throws -> CloudKitHabit {
