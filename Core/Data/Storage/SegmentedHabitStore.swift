...
actor SegmentedHabitStore: ObservableObject {
  ...
  func appendCompletionHistory(habitId: UUID, date: Date, completion: Int) async throws {
    ...
    let historyFileURL = historyDirectoryURL
      .appendingPathComponent("\(habitId.uuidString)_\(monthKey).json")
  ...
  func loadCompletionHistory(
    habitId: UUID,
    from startDate: Date,
    to endDate: Date) async -> [String: Int]
  {
  ...
  func cleanupOldHistory(olderThanMonths: Int = 24) async throws {
    let cutoffDate = Calendar.current
      .date(byAdding: .month, value: -olderThanMonths, to: Date()) ?? Date()
...
enum SegmentedStorageError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .dataSizeExceeded(let current, let limit, let suggestion):
      "Data size exceeded: \(current) bytes > \(limit) bytes limit. \(suggestion)"
