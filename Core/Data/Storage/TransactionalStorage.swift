...
enum TransactionError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .operationFailed(let operation, let error):
      "Storage operation failed: \(operation) - \(error.localizedDescription)"
    case .rollbackFailed(let error):
      "Failed to rollback transaction: \(error.localizedDescription)"
  ...
  var recoverySuggestion: String? {
    ...
    case .rollbackFailed:
      "Data may be in an inconsistent state. Consider manual recovery."
...
final class TransactionManager {
  ...
  func performTransaction(
    _ operations: [StorageOperation],
    using storage: TransactionalStorage) async throws
  {
  ...
  private func createBackup(
    ...
    using storage: TransactionalStorage) async throws -> [String: Data]
  {
  ...
  private func readData(for _: String, using _: TransactionalStorage) async throws -> Data? {
...
// MARK: - AtomicStorageWrapper

/// Wraps any storage implementation with atomic write capabilities
final class AtomicStorageWrapper<T: HabitStorageProtocol>: HabitStorageProtocol,
