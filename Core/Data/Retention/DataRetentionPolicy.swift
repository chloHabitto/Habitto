...
final class DataRetentionManager {
    ...
    private init() {
        ...
        logger.info("DataRetentionManager initialized with policy: \(self.currentPolicy.completionHistoryDays) days retention")
    }
    ...
    func performCleanup() async throws -> CleanupResult {
        ...
        logger.info("Data cleanup completed in \(String(format: "%.2f", duration))s - Cleaned: \(result.totalItemsCleaned) items")
    ...
    private func cleanupCompletionHistory() async throws -> Int {
        ...
        let cutoffDate = Calendar.current.date(byAdding: .day, value: -currentPolicy.completionHistoryDays, to: Date()) ?? Date()
        var cleanedCount = 0
    ...
    private func cleanupDifficultyHistory() async throws -> Int {
        ...
        let cutoffDate = Calendar.current.date(byAdding: .day, value: -currentPolicy.difficultyHistoryDays, to: Date()) ?? Date()
        var cleanedCount = 0
    ...
    private func cleanupUsageHistory() async throws -> Int {
        ...
        let cutoffDate = Calendar.current.date(byAdding: .day, value: -currentPolicy.usageHistoryDays, to: Date()) ?? Date()
        var cleanedCount = 0
...
struct CleanupResult: Codable {
    ...
    var completionHistoryCleaned: Int = 0
    var difficultyHistoryCleaned: Int = 0
    var usageHistoryCleaned: Int = 0
    var notesCleaned: Int = 0
    var habitsCleaned: Int = 0
    
    var totalItemsCleaned: Int {
        return completionHistoryCleaned + difficultyHistoryCleaned + usageHistoryCleaned + notesCleaned + habitsCleaned
    }
}
...
extension DataRetentionPolicy {
    ...
    var isConservative: Bool {
        return completionHistoryDays >= 365 && difficultyHistoryDays >= 180 && usageHistoryDays >= 90
    }
    
    /// Checks if the policy is aggressive (short retention)
    var isAggressive: Bool {
        return completionHistoryDays <= 90 && difficultyHistoryDays <= 60 && usageHistoryDays <= 30
    }
}
