...
class DataRepairUtility: ObservableObject {
  ...
  func performDataRepair() async throws -> RepairSummary {
    ...
    logger
      .info(
        "Data repair completed: \(summary.totalIssuesFound) issues found, \(summary.totalIssuesFixed) fixed")
    return summary
  }
  ...
  private func cleanupOrphanedData(_: [Habit]) async -> RepairResult {
