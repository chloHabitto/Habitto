...
// MARK: - OptimizedHabitStorageManager
...
class OptimizedHabitStorageManager {
  ...
  func migrateFromArrayStorage() {
    ...
    print(
      "🔄 OptimizedHabitStorageManager: Migrating \(oldHabits.count) habits from array storage...")
  ...
  func getStorageStats() -> (totalHabits: Int, totalKeys: Int, cacheSize: Int) {
    ...
    let totalKeys = userDefaults.dictionaryRepresentation().keys
      .filter { $0.hasPrefix(habitPrefix) }.count
  ...
  private func capHabitHistory(_ habit: Habit) -> Habit {
    ...
    if cappedHabit.completionHistory.count > maxHistoryDays {
      ...
      print(
        "🔧 OptimizedHabitStorageManager: Capped completion history for '\(habit.name)' to \(maxHistoryDays) days")
    }
    ...
    if cappedHabit.difficultyHistory.count > maxHistoryDays {
      ...
      print(
        "🔧 OptimizedHabitStorageManager: Capped difficulty history for '\(habit.name)' to \(maxHistoryDays) days")
    }
    ...
    if cappedHabit.actualUsage.count > maxHistoryDays {
      ...
      print(
        "🔧 OptimizedHabitStorageManager: Capped actual usage history for '\(habit.name)' to \(maxHistoryDays) days")
    }
...
extension OptimizedHabitStorageManager {
  ...
  var needsMigration: Bool {
    userDefaults.data(forKey: "SavedHabits") != nil && userDefaults.data(forKey: habitIdsKey) == nil
  }
