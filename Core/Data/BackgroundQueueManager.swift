...
class BackgroundQueueManager: ObservableObject {
    ...
    func execute<T>(_ task: @escaping () throws -> T, completion: @escaping (Result<T, Error>) -> Void) {
        Task { @MainActor in
            activeOperations += 1
            isProcessing = true
        }
    ...
    func executeSerial<T>(_ task: @escaping () throws -> T, completion: @escaping (Result<T, Error>) -> Void) {
        Task { @MainActor in
            activeOperations += 1
            isProcessing = true
        }
    ...
    func executeOnMain<T>(_ task: @escaping () throws -> T, completion: @escaping (Result<T, Error>) -> Void) {
        DispatchQueue.main.async {
            do {
                let result = try task()
                completion(.success(result))
...
extension BackgroundQueueManager {
    ...
    func executeOnBackgroundReturnOnMain<T>(_ task: @escaping () throws -> T, completion: @escaping (Result<T, Error>) -> Void) {
