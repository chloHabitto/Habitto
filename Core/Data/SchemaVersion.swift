...
enum SchemaVersion {
  ...
  static func isValid(_ version: Int) -> Bool {
    versions.keys.contains(version)
  }
...
class SchemaMigrationExecutor {
  ...
  private func performDataTransform(_: (Any) -> Any) async throws {
    // Implementation would depend on specific data transformation needs
    logger.info("🔄 Executing data transformation")
  }

  private func performDataValidation(_: (Any) -> Bool) async throws {
...
enum SchemaMigrationError: Error, LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .unsupportedMigration(let from, let to):
      "Migration from version \(from) to \(to) is not supported"
    case .stepFailed(let step, let error):
      "Migration step \(step) failed: \(error.localizedDescription)"
    case .validationFailed(let step, let reason):
      "Migration step \(step) validation failed: \(reason)"
    case .dataCorruption(let details):
      "Data corruption detected during migration: \(details)"
    case .insufficientStorage(let required, let available):
      "Insufficient storage for migration. Required: \(required), Available: \(available)"
    }
  }
}
