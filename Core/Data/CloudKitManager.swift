...
enum CloudKitError: Error, LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case .notConfigured:
      "CloudKit is not configured for this app"
    case .syncFailed(let error):
      "CloudKit sync failed: \(error.localizedDescription)"
    case .conflictDetected:
    ...
    case .invalidRecord:
      "Invalid CloudKit record format"
    }
  }
}
...
class CloudKitManager: ObservableObject {
  ...
  func saveCustomRecord(_ record: CKRecord) async throws -> CKRecord {
    ...
    if VacationManager.shared.isActive {
      ...
      throw CloudKitError.syncFailed(NSError(
        domain: "VacationMode",
        code: 1,
        userInfo: [NSLocalizedDescriptionKey: "Cloud operations paused during vacation mode"]))
    }
