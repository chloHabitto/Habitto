...
// MARK: - AddHabitCreationDateMigration
...
struct AddHabitCreationDateMigration: MigrationStep {
  ...
  func execute() async throws -> MigrationResult {
    ...
    for habit in habits {
      // Check if habit needs creationDate (since createdAt already exists, this migration is not
      // needed)
    ...
    if !updatedHabits.isEmpty {
      ...
      print(
        "✅ AddHabitCreationDateMigration: Processed \(updatedHabits.count) habits (creationDate already exists as createdAt).")
      return .success
    } else {
      UserDefaults.standard.set(true, forKey: "AddHabitCreationDateMigrationCompleted")
  ...
  func rollback() async throws {
    print(
      "🔄 AddHabitCreationDateMigration: Rollback not implemented as it's a non-destructive addition.")
    UserDefaults.standard.removeObject(forKey: "AddHabitCreationDateMigrationCompleted")
  }
}

// MARK: - NormalizeHabitGoalMigration
...
struct NormalizeHabitGoalMigration: MigrationStep {
  ...
  func rollback() async throws {
    print(
      "🔄 NormalizeHabitGoalMigration: Rollback not implemented as it's a non-destructive normalization.")
    UserDefaults.standard.removeObject(forKey: "NormalizeHabitGoalMigrationCompleted")
  }
}

// MARK: - CleanUpInvalidDataMigration
...
struct CleanUpInvalidDataMigration: MigrationStep {
  ...
  func execute() async throws -> MigrationResult {
    ...
    if removedCount > 0 {
      ...
      print(
        "✅ CleanUpInvalidDataMigration: Removed \(removedCount) invalid habits, kept \(habits.count) valid habits.")
      return .success
    } else {
      UserDefaults.standard.set(true, forKey: "CleanUpInvalidDataMigrationCompleted")
  ...
  func canRollback() -> Bool {
    false // Destructive operation, cannot rollback
  }
