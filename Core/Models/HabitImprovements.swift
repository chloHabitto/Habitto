...
extension Habit {
  ...
  var normalizedDifficultyLog: [NormalizedDayLog] {
    difficultyHistory.compactMap { dateKey, value in
      NormalizedDayLog.from(dateKey: dateKey, value: value)
    }
  }
  ...
  var normalizedUsageLog: [NormalizedDayLog] {
    actualUsage.compactMap { dateKey, value in
      NormalizedDayLog.from(dateKey: dateKey, value: value)
    }
  }
  ...
  var scheduleDisplayName: String {
    ...
    case .everyNDays:
      ...
      let numbers = schedule.components(separatedBy: CharacterSet.decimalDigits.inverted)
        .compactMap { Int($0) }
