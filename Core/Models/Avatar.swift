...
class AvatarManager: ObservableObject {
    ...
    private func loadAvatarFromUserAccount() {
        ...
        selectedAvatar = avatar
        print("✅ AvatarManager: Loaded avatar for authenticated user: \(avatar.displayName) (ID: \(avatar.id))")
    }
    ...
    private func saveSelectedAvatar() {
        ...
        if isUserAuthenticated() {
            ...
            print("💾 AvatarManager: Saved avatar for authenticated user with key: \(userKey) - \(selectedAvatar.displayName)")
        } else {
    ...
    func handleAuthStateChange(_ authState: AuthenticationState) {
        ...
        case .authenticated:
            ...
            if let user = authManager.currentUser {
                print("👤 AvatarManager: User authenticated - UID: \(user.uid), Email: \(user.email ?? "no-email")")
            }
    ...
    private func saveAvatarToUserAccount(_ avatar: Avatar) {
        ...
        print("🔐 AvatarManager: Would save avatar to user account (Firebase Storage integration needed)")
    }
